...
public class INDArrayTest {
  ...
  public void testMiniBatch() {
    ...
    INDArray example = d0.get(0).getFeatures().get(point(0), all(), point(0));
    assertEquals(Nd4j.create(new float[]{0,0,0}), example);
    ...
    for(int batchNum=0; batchNum<num; batchNum++ ) {
      for(int exampleNum=0; exampleNum<examplesSize; exampleNum++) {
        ...
        d0.getFeatures().get(point(batchNum), all(), point(exampleNum)).assign(labels);
        //d0.getFeatures().getRow(batchNum).getColumn(exampleNum).assign(labels); // deprecated option from Dl4j-beta3
			}
		}
    ...
    example = d1.get(0).getFeatures().get(point(0), all(), point(0));
    assertEquals(Nd4j.create(new float[]{0,1,2}), example);
    example = d1.get(0).getFeatures().slice(0, 0).slice(0, 1);
    assertEquals(Nd4j.create(new float[]{0,1,2}), example);
    example = d1.get(1).getFeatures().get(point(0), all(), point(0));
    assertEquals(Nd4j.create(new float[]{100,101,102}), example);
    example = d1.get(1).getFeatures().slice(0, 0).slice(0, 1);
    assertEquals(Nd4j.create(new float[]{100,101,102}), example);
    
    assertArrayEquals(new long[]{1, labelSize, examplesSize}, d0.get(0).getFeatures().shape());
    example = d0.get(0).getFeatures().get(point(0), all(), point(0));
    assertEquals(Nd4j.create(new float[]{0,1,2}), example);
    example = d0.get(0).getFeatures().slice(0, 0).slice(0, 1);
    assertEquals(Nd4j.create(new float[]{0,1,2}), example);
    example = d0.get(1).getFeatures().get(point(0), all(), point(0));
    assertEquals(Nd4j.create(new float[]{100,101,102}), example);
    example = d0.get(1).getFeatures().slice(0, 0).slice(0, 1);
    assertEquals(Nd4j.create(new float[]{100,101,102}), example);
